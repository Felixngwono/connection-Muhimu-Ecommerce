...
def customer_products_view(request):
    products=models.Product.objects.all()
    return render(request,'ecom/customer_product.html',{'products':products})
...
@login_required(login_url='customerlogin')
def customer_add_product_view(request):
    productForm=forms.ProductForm()
    if request.method=='POST':
        productForm=forms.ProductForm(request.POST, request.FILES)
        if productForm.is_valid():
            productForm.save()
        return HttpResponseRedirect('customer_product')
    return render(request,'ecom/customer_add_product.html',{'productForm':productForm})
